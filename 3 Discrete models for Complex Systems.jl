...
md" # 3. Dynamics in complex systems 

A key aspect of complex systems is their dynamics, how they change in time. Analytical solution of systems of interactions are often impossible to achieve, given the number of variables and the interactions, so, when we focus on the dynamics. we have to use modeing. 
...
A first approach is to monitor at the state of a system at different times, and then compare the difference between these time points to see its dynamics. We will illustrate this approach using a very simple system of just one species that is proliferating and/or dying, and therefore the number of individuals in the population changes as time progresses. This can be applied to animals, cancer cells, bacteria...
"
...
solving these anallitically is often not possible, so we have to approach it by trying to solve it numerically: 

As a startyinbg point, lets make $r(p_t)$  a linear function. Suppose that a population of p = 1 experiences a growth factor of r(1) = 1.1. Assume also that the equilibrium population size is 50, so that the corresponding growth factor is r(50) = 1.
...
```math
Slope=\frac{0.9 - 1.1}{100-0}=\frac{- 0.2}{100}=- 2 \cdot 10^{-3}\\
p(0)=1.1
```
...
function constrained_growth_3(p, r; dt=0.01)
traj = []
...
(b) If $P_0$=1500, Calculate the population after 3 weeks. find $\lim_{t \to \infty}  P_t$. Is the solution curve increasing or decreasing? Justify your answer. Sketch the graph of $P_t$.

(c) If $P_0$=3000, Calculate the population after  3 weeks. find $\lim_{t \to  \infty}  P_t$. Is the solution curve increasing or decreasing? Justify your answer. Sketch the graph of $P_t$.

(d) How many cells are in the tumour when the population is growing the fastest? Justify your answer. 

"

# ╔═╡ 2958fbbc-86e9-47fd-8097-9a8675610998
md"Solutions
(a) 2500; increasing (b) 2500; increasing (c) 2500; decreasing (d) 1250
"
...
begin
	rr_slide = @bind rr html"<input type=range min=0.5 max=4.0 step=0.1>"
...
md" It’s called the logistic map because it maps the population value at any time step to its value at the next time step:
...
md" ## Equilibrium in the logistic map
...
condition for equilibrium is that the next point is equial to teh previous time point
...
where μ>1.We will refer to $x = 0$ as the zero equilibrium, and $x  = \frac{\mu - 1}{\mu}$ as the positive equilibrium. Now that we established where the equilibrium points are located, we can examine what happens when we modify the constant, μ, and the initial value of the sequence. When 0 < μ ≤ 1, we will only have one equilibrium point, x = 0. Moreover, the recursive sequence will converge to x = 0. 

When 1 < μ < 3, the sequence will converge to the positive equilibrium, and diverge from the zero equilibrium. 


"

# ╔═╡ 807ff1f9-5ac1-4c4f-9602-2fb0ac1e7f8e
x_1=collect(0:0.01:1)

# ╔═╡ 248a2470-60e3-4abf-8c25-e2b1c059c5ed
begin
	rrr_slide = @bind rrr html"<input type=range min=0.5 max=4.0 step=0.1>"
	
	md"""
	**Set the growth rate?**
	
	value of r: $(rrr_slide)
...
begin
	plot(x_1, rrr .* x_1 .* (1 .− x_1),xlabel=("p_current"),ylims = (0,1),ylabel=("p_next"),title=("Constrained growth, r=$rrr"))
